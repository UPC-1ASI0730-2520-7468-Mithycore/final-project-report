...
|----------|--------------------------------------------|---------------|-----------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------:|---------------|-------------------------|
| US01     | Registro de paciente                       | T01.1         | Implementar modelo Patient y hashing de contraseña        | Definir entidad Patient y añadir campo `passwordHash`; implementar hashing seguro (Argon2/Bcrypt) en el servicio de registro.                                      |        5   |               |                         |
| US01     | Registro de paciente                       | T01.2         | Endpoint POST `/api/v1/patients/register`                | Implementar endpoint REST que reciba DTO, valide duplicados (email/phone) y cree el paciente devolviendo `201` con `id`.                                           |        4   |               |                         |
| US02     | Visualizar perfil del paciente             | T02.1         | Servicio: Obtener perfil de paciente por ID              | Crear servicio que recupere paciente y mapee a DTO excluyendo campos sensibles.                                                                                    |        3   |               |                         |
| US02     | Visualizar perfil del paciente             | T02.2         | Endpoint GET `/api/v1/patients/{patientId}`              | Exponer endpoint protegido que devuelva el perfil; validar permisos (paciente o neurólogo autorizado).                                                             |        3   |               |                         |
| US03     | Actualizar perfil parcial del paciente     | T03.1         | DTO y validaciones para actualización parcial            | Definir DTO parcial `UpdatePatientProfileRequest` con campos opcionales y validaciones (email/tel).                                                                 |        2   |               |                         |
| US03     | Actualizar perfil parcial del paciente     | T03.2         | Endpoint PATCH `/api/v1/patients/{patientId}` y servicio | Crear endpoint y servicio que aplique cambios parciales (merge), prevenga cambio de rol y persista.                                                                |        4   |               |                         |
| US04     | Vincular paciente a un neurólogo           | T04.1         | Repositorio: persistir vínculo paciente–neurólogo        | Implementar método para crear relación `patient_neurologists` evitando duplicados.                                                                                 |        3   |               |                         |
| US04     | Vincular paciente a un neurólogo           | T04.2         | Endpoint POST `/api/v1/patients/{patientId}/neurologists/{neurologistId}` | Endpoint que valida existencia de entidades y crea la solicitud/vínculo en estado `requested`.                                                                     |        3   |               |                         |
| US05     | Crear perfil profesional de neurólogo      | T05.1         | Modelo Neurologist y validaciones de licencia            | Definir entidad con `licenseNumber`, `specialties`, `verificationStatus`; validar formato de licencia.                                                             |        4   |               |                         |
| US05     | Crear perfil profesional de neurólogo      | T05.2         | Endpoint POST `/api/v1/neurologists`                     | Endpoint para crear perfil profesional; devuelve `201 Created` con `neurologistId` y `verificationStatus`.                                                         |        3   |               |                         |
| NEU-002  | Listar pacientes asociados                 | T-NEU002-1    | Repositorio: Obtener pacientes por `neurologistId`       | Método que retorna pacientes asociados con metadata (`assignedAt`, `status`).                                                                                       |        3   |               |                         |
| NEU-002  | Listar pacientes asociados                 | T-NEU002-2    | Endpoint GET `/api/v1/neurologists/{neurologistId}/patients` | Endpoint protegido que devuelve la lista; admite filtro simple `?status=`.                                                                                         |        3   |               |                         |
| NEU-003  | Actualizar perfil profesional              | T-NEU003-1    | DTO `UpdateNeurologistProfileRequest`                    | Definir DTO con campos editables opcionales (`specialties`, `bio`, `phone`, `officeAddress`).                                                                      |        2   |               |                         |
| NEU-003  | Actualizar perfil profesional              | T-NEU003-2    | Endpoint PATCH `/api/v1/neurologists/{neurologistId}`    | Endpoint para aplicar actualizaciones parciales; validar ownership (propio neurólogo o admin).                                                                      |        3   |               |                         |
| NEU-005  | Gestionar solicitudes de asociación        | T-NEU005-1    | Repositorio para solicitudes de asociación               | Crear tabla/entidad `AssociationRequest` y métodos para CRUD básico (status transitions).                                                                          |        3   |               |                         |
| NEU-005  | Gestionar solicitudes de asociación        | T-NEU005-2    | Endpoint PATCH `/api/v1/neurologists/{neurologistId}/requests/{requestId}` | Endpoint para aceptar/rechazar solicitud; al aceptar crear relación `patient_neurologists`.                                                                      |        4   |               |                         |
